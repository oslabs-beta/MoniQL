const { userPool, connectToPool } = require('../models/db');
...
ON 
    ...
    db.end()
      .then(() => console.log('db connection closed in dbController.getDB'))
      .catch(err => console.error('Error closing db connection in dbController.getDB:', err));
    return next()
  } catch (err) {
    return next(err)
  }
...
// took this out of the function, couldn't look at it anymore -a
//     const query = `SELECT
//     table_info.table_name,
//     table_info.columns,
//     fk_info.foreign_keys
