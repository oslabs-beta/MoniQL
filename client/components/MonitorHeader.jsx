import { Typography, Box, useTheme, Select, FormControl, InputLabel, MenuItem, Chip } from '@mui/material';
...
const MonitorHeader = () => {
  ...
  monitors.forEach(monitor => {
    if(columnsOnTablesWithMonitors[monitor.parameters.table] && !columnsOnTablesWithMonitors[monitor.parameters.table].includes(monitor.parameters.column)){
      columnsOnTablesWithMonitors[monitor.parameters.table].push(monitor.parameters.column);
    }
  });
    ...
    <div>
      ...
      <Box>
        <FormControl fullWidth>
          ...
          >
            {tablesWithMonitors.length && tablesWithMonitors.map((table, i) => {
              return <MenuItem key={i} value={table}>{table}</MenuItem>
            })}
          </Select>
        </FormControl>
        <FormControl fullWidth>
          ...
          >
            {columnsOnTablesWithMonitors[selectedTable] && columnsOnTablesWithMonitors[selectedTable].map((column, i) => {
              return <MenuItem key={i} value={column}>{column}</MenuItem>
            })}
          </Select>
        </FormControl>
        <FormControl fullWidth>
